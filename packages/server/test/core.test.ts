...
describe("first milestone", () => {
  ...
  test("queues a follow-up after completion", async () => {
    ...
    expect(server.store.events(id).filter(event => event.type === "completed")).toHaveLength(2);
  });
  ...
  test("restarts with steering after aborting foreground work", async () => {
    ...
    await running;
    expect(server.store.events(id).filter(event => event.type === "aborted")).toHaveLength(1);
    expect(server.store.events(id).filter(event => event.type === "completed")).toHaveLength(1);
  });

  test("persists an explicit provider/model selection for resume", async () => {
    const { dir, server } = harness(); const id = server.createSession();
    await server.command(id, { type: "configure", provider: "openai-compatible", model: "test-model", baseUrl: "http://127.0.0.1:1/v1" });
    expect(new HarnessServer(new SessionStore(join(dir, "state.sqlite")), dir).store.modelConfig(id)).toEqual({ provider: "openai-compatible", model: "test-model", baseUrl: "http://127.0.0.1:1/v1" });
  });
  ...
  test("streams an OpenAI-compatible model through the Pi agent loop and core tools", async () => {
    ...
    const provider = Bun.serve({ port: 0, fetch: async request => {
      ...
      return new Response(new ReadableStream({ start(controller) { for (const chunk of body) controller.enqueue(new TextEncoder().encode(chunk)); controller.close(); } }), { headers: { "content-type": "text/event-stream" } });
    }});
    try {
      const { dir, server } = harness(); writeFileSync(join(dir, "note.txt"), "hello"); const id = server.createSession();
      await server.command(id, { type: "configure", provider: "openai-compatible", model: "test-model", baseUrl: `http://127.0.0.1:${provider.port}/v1` });
      ...
      expect(events.some(event => event.type === "tool-call" && event.name === "read")).toBe(true);
      expect(events.some(event => event.type === "tool-result" && event.output === "hello")).toBe(true);
      expect(events.some(event => event.type === "usage" && event.totalTokens === 9)).toBe(true);
    } finally { provider.stop(true); delete process.env.HARNESS_OPENAI_API_KEY; }
  });
  ...
  test("aborts an in-flight OpenAI-compatible Pi request", async () => {
    ...
    const provider = Bun.serve({ port: 0, fetch: request => new Promise<Response>(resolve => {
      started();
      request.signal.addEventListener("abort", () => resolve(new Response("cancelled", { status: 499 })), { once: true });
    }) });
    try {
      const { server } = harness(); const id = server.createSession();
      await server.command(id, { type: "configure", provider: "openai-compatible", model: "test-model", baseUrl: `http://127.0.0.1:${provider.port}/v1` });
      ...
      await Promise.race([running, new Promise((_, reject) => setTimeout(() => reject(new Error("model cancellation timed out")), 1_000))]);
      expect(server.store.events(id).filter(event => event.type === "aborted")).toHaveLength(1);
    } finally { provider.stop(true); delete process.env.HARNESS_OPENAI_API_KEY; }
  });
});
