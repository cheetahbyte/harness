...
describe("HTTP event stream", () => {
  test("stays connected past Bun's default idle timeout", async () => {
    const dir = mkdtempSync(join(tmpdir(), "harness-http-test-")); paths.push(dir);
    const server = serveHarness({ port: 0, workspace: dir, databasePath: join(dir, "state.sqlite") });
    try {
      ...
      await fetch(`${base}/sessions/${sessionId}/commands`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ type: "prompt", text: "/read missing" }) });
