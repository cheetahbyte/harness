...
export class SessionStore {
  ...
  constructor(path = ".harness/harness.sqlite") {
    ...
    this.db.run("CREATE TABLE IF NOT EXISTS session_settings (session_id TEXT PRIMARY KEY, model_config TEXT NOT NULL)");
  }
  ...
  modelConfig(sessionId: string): HarnessModelConfig | undefined {
    const row = this.db.query("SELECT model_config FROM session_settings WHERE session_id = ?").get(sessionId) as { model_config: string } | null;
    return row ? JSON.parse(row.model_config) as HarnessModelConfig : undefined;
  }

  setModelConfig(sessionId: string, config: HarnessModelConfig): void {
    this.db.query("INSERT OR REPLACE INTO session_settings (session_id, model_config) VALUES (?, ?)").run(sessionId, JSON.stringify(config));
  }
